...
def train_model(data, X_train, X_test, y_train, y_test):
    try:
        ...
        # Save the Model
        model_filename = config.MODELS_ROOT + config.CHURN_LTSM_MODEL
...
def evaluate_model(model, X, y, X_test, y_test):
    try:
        ...
        # Print coefficients (feature importances for interpretability)
        print(f"Feature Coefficients:")
        for idx, col in enumerate(X.columns):
            print(f"{col}: {model.layers[1].get_weights()[0][idx][0]:.4f}")
    except Exception as e:
